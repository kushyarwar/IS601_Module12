"""
Integration tests for Module 12: User & Calculation Routes.
Covers registration, login, and full BREAD operations for calculations.
"""


# ── Helpers ───────────────────────────────────────────────────────────────────

def _register(client, username="testuser", email=None, password="pass123"):
    if email is None:
...
def test_login_success(client):
    ...
    res = client.post("/users/login", json={"username": "frank", "password": "mypassword"})
...
def test_login_wrong_password(client):
    ...
    res = client.post("/users/login", json={"username": "grace", "password": "wrongpass"})
    assert res.status_code == 401


def test_login_nonexistent_user(client):
    res = client.post("/users/login", json={"username": "nobody", "password": "pass"})
    assert res.status_code == 401


def test_login_response_omits_password(client):
    _register(client, "henry", password="secret")
    res = client.post("/users/login", json={"username": "henry", "password": "secret"})
...
def test_login_verifies_hashed_password_in_db(client, db_session):
    ...
    assert user.password_hash != "mypass"  # stored as hash, not plaintext
